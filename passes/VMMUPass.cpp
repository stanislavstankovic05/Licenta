...
{
    return {
        LLVM_PLUGIN_API_VERSION, "VMMUPass", "v1.0",
        [](PassBuilder &PB)
        {
            /* Run before loop unrolling/vectorization so the hoisting
             * matcher sees canonical counted loops (base + simple IV).
             * At OptimizerLast the loops are already unrolled and hoisting
             * fails -> O(N) per-access checks. */
