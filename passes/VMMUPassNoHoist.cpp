...
struct VMMUPass : PassInfoMixin<VMMUPass>
{
    PreservedAnalyses run(Function &F, FunctionAnalysisManager &AM)
    {
        ...
        /* NO-HOIST variant: hoisting disabled on purpose so every access
         * gets a per-element vmmu_check. Used as the baseline against the
         * hoisting build. (void casts silence unused-analysis warnings.) */
        (void)all_loops;
        (void)SE;
...
{
    return {
        LLVM_PLUGIN_API_VERSION, "VMMUPassNoHoist", "v1.0",
        [](PassBuilder &PB)
        {
            /* O0 variant.
             *
